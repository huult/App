...
function ProductMarketingWindowManager({topmostRouteName}: ProductMarketingWindowManagerProps) {
    ...
    // The admin CTA's Vendors-vs-More-features branch reads policy.connections (via hasVendorFeature), which is only
    // hydrated for the active policy at startup; prefetch it here so a fallback admin workspace routes correctly too.
    usePolicyConnectionsPrefetch(targetAdminPolicy, isVendorMatchingBetaEnabled);
    ...
    const completeCta = () => {
        ...
        persistDismissal();
        Navigation.navigate(variant.getCtaRoute({policyID: targetAdminPolicy?.id, policy: targetAdminPolicy, isVendorMatchingBetaEnabled}));
    };
