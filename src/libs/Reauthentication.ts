...
import redirectToSignIn, {redirectToSAMLSignIn} from './actions/SignInRedirect';
...
function reauthenticate(command = ''): Promise<boolean> {
    ...
    return hasReadRequiredDataFromStorage().then(() => {
        ...
        if (account?.isSAMLRequired) {
            Log.info('[Reauthenticate] SAML required account detected. Redirecting to SAML sign in.');
            setIsAuthenticating(false);
            return redirectToSAMLSignIn().then(() => false);
        }
